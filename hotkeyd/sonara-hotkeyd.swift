// sonara-hotkeyd.swift
// Sonara Phase 2 global-hotkey daemon.
//
// Reads ~/.sonara/hotkeyd.resolved.json — an array of
//   { "action": String, "keyCode": Int, "modifiers": Int, "message": String }
// produced by sonara.keymap.write_resolved(). For each entry it registers a
...
// daemon's port + token from ~/.sonara/daemon.lock and writes the session
...
//
// Build: swiftc hotkeyd/sonara-hotkeyd.swift -o ~/.sonara/sonara-hotkeyd
// Run:   the com.sonara.hotkeyd LaunchAgent (Aqua session, .accessory policy).
...
func sonaraDir() -> String {
    return (NSHomeDirectory() as NSString).appendingPathComponent(".sonara")
}

func resolvedPath() -> String {
    return (sonaraDir() as NSString).appendingPathComponent("hotkeyd.resolved.json")
}
...
// Read ~/.sonara/daemon.lock for the daemon's ephemeral port + session token.
func lockInfo() -> (port: UInt16, token: String)? {
    let path = (sonaraDir() as NSString).appendingPathComponent("daemon.lock")
