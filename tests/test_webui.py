...
def test_settings_page_has_summary_styles_ui():
    ...
    # the fast trio only, each speed descriptor unique ("fastest" twice reads
    # as nonsense)
    import re
    descriptors = re.findall(r'label: "[^"(]+\(([^)]+)\)"', page)
    assert descriptors == ["fastest", "faster", "fast"]
    for gone in ("gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.5", '"sonnet"', '"opus"'):
