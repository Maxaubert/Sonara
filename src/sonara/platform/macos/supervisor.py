...
LAUNCH_AGENT_LABEL = "com.sonara.speechd"
LAUNCH_AGENT_PATH = os.path.expanduser(
    "~/Library/LaunchAgents/com.sonara.speechd.plist")
...
def _launcher_path() -> str:
    return os.path.join(os.path.expanduser("~"), ".local", "bin", "sonara")
...
class MacSupervisorBackend(SupervisorBackend):
    ...
    def launchagent_plist(self, python_executable: str, src_path: str,
        ...
        return self.plist(
            LAUNCH_AGENT_LABEL,
            [python_executable, "-m", "sonara.daemon"],
            log_path,
    ...
    def launch_spec(self):
        ...
        shim = os.path.join(paths.repo_root(), "bin", "sonara-daemon")
    ...
    def place_launcher(self, plugin_root: str) -> str:
        """Write an executable ~/.local/bin/sonara that execs the plugin bin/sonara.

        The plugin path is baked in and shell-quoted so a path with spaces is safe.
        Overwrites any prior Sonara-owned launcher. Returns the launcher path.
        """
        target = os.path.join(plugin_root, "bin", "sonara")
        wrapper = (
            "#!/usr/bin/env bash\n"
    ...
    def post_install_notes(self) -> None:
        ...
        print("")
        print("Enable the Sonara plugin in Claude Code, then run 'sonara doctor'.")
        print("  - Per session: claude --plugin-dir {0}".format(plugin_root))
        print("  - Or enable 'sonara' from the /plugin menu (local marketplace).")
        if not _local_bin_on_path():
            print('Add ~/.local/bin to your PATH so `sonara` works in every shell:')
    ...
    def doctor_rows(self) -> list:
        ...
        hotkeyd LaunchAgent loaded, sonara launcher.
        """
        ...
        rows.append(("hotkeyd binary", hk_exists,
                     hk_bin if hk_exists
                     else "missing: {0} (run 'sonara install')".format(hk_bin)))
        ...
        rows.append(("speechd LaunchAgent loaded", speechd_loaded,
                     ...
                     else "not loaded (run 'sonara install')"))
        ...
        rows.append(("hotkeyd LaunchAgent loaded", hotkeyd_loaded,
                     ...
                     else "not loaded (build CLT then 'sonara install')"))

        # sonara launcher + PATH
        ...
        if launcher_ok and on_path:
            ...
            detail = "missing (run 'sonara install')"
        rows.append(("sonara launcher", launcher_ok and on_path, detail))
