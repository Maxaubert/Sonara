...
``sonara.kokoro`` engine to WAV bytes and played by spawning ``afplay`` on a
...
def _sweep_stale_wavs(max_age_s: float = 300.0) -> None:
    ...
    still be playing is never deleted) and only our own ``sonara-tts-*`` prefix
...
class MacTtsBackend(TtsBackend):
    ...
    def list_voices(self) -> "List[str]":
        ...
        the venv, so gating on is_installed alone hid the voices from `sonara voice`."""
